...
open class SCLAlertView: UIViewController {
    ...
    @objc func buttonTapDown(_ btn:SCLButton) {
        btn.backgroundColor = COLOR_PRINCIPAL
    }
    ...
    open func showError(_ title: String, subTitle: String, closeButtonTitle:String?=nil, duration:TimeInterval=0.0, colorStyle: UInt=0xC1272D, colorTextButton: UInt=0xFFFFFF) {
        showTitle(title, subTitle: subTitle, duration: duration, completeText:closeButtonTitle, style: .error, colorStyle: colorStyle, colorTextButton: colorTextButton)
    }
